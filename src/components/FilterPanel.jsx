              ...
              return (
                <div key={attr.id} onDragOver={(e) => onQDragOver(e, i)} onDrop={(e) => onQDrop(e, i)} style={{ borderTop: isOver ? `2px solid ${PAI.indigo}` : '2px solid transparent' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '10px 0', borderBottom: '1px solid var(--shell-border)', opacity: isDragging ? 0.4 : 1, transition: 'opacity 120ms' }}>
                    ...
                    <button onClick={() => deleteAttr(attr.id)} disabled={liveAttrs.length <= 1} style={{ width: 24, height: 24, padding: 0, background: 'none', border: 'none', cursor: liveAttrs.length <= 1 ? 'not-allowed' : 'pointer', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', color: liveAttrs.length <= 1 ? 'var(--pai-disabled)' : 'var(--pai-crit-fg)', borderRadius: 4, flexShrink: 0 }}>
