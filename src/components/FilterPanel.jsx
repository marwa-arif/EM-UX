...
const GFSidePanel = forwardRef(function GFSidePanel({ onEntitySelect, selectedEntityId, onApply, onConnectionsChange, onDirtyChange, hidden, entityAttrFilters, onEntityAttrFiltersChange }, ref) {
  ...
  const hasAnyPotentialConnections = visibleRelationEdges.length > 0;
  ...
  // Notify parent of filter count / dirty state so footer Apply & Reset buttons reflect it
  useEffect(() => {
      ...
      <div className="gf-side-panel__bottom">
        ...
        <div className="gf-bottom-tree">
          ...
          {allConnections.length === 0 && Object.values(entityAttrFilters).every(f => Object.keys(f || {}).length === 0)
            ? (
              <p className="gf-bottom-empty">
                {hasAnyPotentialConnections
                  ? 'Select a node, then click a connected node to build a path filter'
                  : 'No related entities are currently shown — use Add / Hide Entity to bring related entities into view'}
              </p>
            )
